...
#      message(glue("setRvUmv end, rv$giniUmv: {rv$giniUmv}"))
  ...
  fTabStat <- function(jahr) {
    tStat <- c(
      "Current year Nr.:" = rv$lfdJahr,
      "Total wealth begin of current year:" = fmtBetrag(rv$gesamtVermoegenAnf),
      "Total wealth end of current year:" = fmtBetrag(rv$gesamtVermoegenEnd),
      "Total wealth after redistribution:" = fmtBetrag(rv$gesamtVermoegenUmv),
      "Greatest wealth end of current year:" = fmtBetrag(max(rv$lastWeekVermoeg)),
      "Greatest wealth after redistribution:" = fmtBetrag(max(rv$nachSteuern)),
      "Smallest wealth end of current year:" = fmtBetrag(min(rv$lastWeekVermoeg)),
      "Smallest wealth after redistribution:" = fmtBetrag(min(rv$nachSteuern))
    )
    return (tStat)
  }
  ...
  fTabEndStat <- function(jahr) {
    tEndStat <- c(
      ...
      "Average:" = fmtBetrag(rv$weekMean[anzWeek]),
...
## reactive output
  ...
  output$outPlotVerlauf <- renderPlot({
    ggplot(data = rv$s, aes(x = 1:anzWeek)) +
      ...
      labs(y = "Wealth (log10)", x = "Weeks", title = "Weekly progression minimum, median, average, maximum of wealth", ) +
      scale_color_manual(
        values = c("firebrick", "darkgreen", "red", "green"),
        name = "Legend:",
        labels = c("Maximum", "Median", "Average", "Minimum")
      ) +
  ...
  output$outPlotVerlaufGes <- renderPlot({
    ggplot(data = rv$s, aes(x = 1:anzWeek)) +
      ...
      scale_y_log10(labels = scales::dollar) +
      labs(y = "Total Wealth (log10)", x = "Weeks", title = "Weekly progression sum of wealth of all persons") +
      scale_color_manual(
        values = c("firebrick", "darkgreen", "red", "green"),
        name = "Legend:",
        labels = c("Sum", "Median", "Mean", "Min")
      ) +
      theme(legend.position = "bottom")
  })
  output$outPlotLorenzAnf <- renderPlot({
    plot(rv$lorenzAnf,
         main = "Lorenz-Curve at the begin of current year",
         xlab = "Share of people in whole population (percentile)",
         ylab = "Share of total wealth", col = "red")
  })
  output$outPlotLorenzEnd <- renderPlot({
    plot(rv$lorenzEnd,
         main = "Lorenz-Curve at the end of current year",
         xlab = "Share of people in whole population (percentile)",
         ylab = "Share of total wealth", col = "red")
  })
  output$outPlotLorenzUmv <- renderPlot({
    plot(rv$lorenzUmv,
         main = "Lorenz-Curve after redistribution",
         xlab = "Share of people in whole population (percentile)",
         ylab = "Share of total wealth", col = "red")
  })
